...
The AIDD MCP memory system (27 tools across 10 modules) uses a dual-storage architecture:
...
- **Confidence Decay**: `aidd_pattern_false_positive` tool reduces pattern confidence by 15% per report. Auto-deactivate learned patterns at useCount < 2.
...
Both PRs implemented successfully:
- **PR 1** (SQLite Consolidation): 16 tables, ~30 StorageBackend methods, JsonBackend deleted, schema checksum, WAL checkpoint, FTS5 pruning
- **PR 2** (Pattern Killer + Auto-Learning): 6 tools, HookBus with 5 subscribers, 7 evolution detectors, insights.md + state-dump.sql auto-generation
- **69 total tools**: 17 core + 33 memory (27 existing + 6 pattern-killer) + 19 tools
- **Memory chunk**: 160.15 KB
- Build: `pnpm mcp:typecheck && pnpm mcp:build` — 5/5 pass
- Engine: `pnpm mcp:check` — ON
