...
- **PR 2**: Add Pattern Killer module (6 tools) + HookBus + auto-learning + model fingerprinting.
...
### PR 1 Checks
- [x] `pnpm mcp:typecheck` — 5/5 pass
- [x] `pnpm mcp:build` — 5/5 pass
- [x] `pnpm mcp:check` — Engine ON
- [x] No readJsonFile/writeJsonFile in module code
- [x] `.aidd/data.db` contains all 16 tables (incl. `meta`)
- [x] Schema checksum stored in `meta` table
- [x] WAL checkpoint called on session end
- [x] All 16 memory tools functional

### PR 2 Checks
- [x] 6 pattern-killer tools registered (audit, list, add, stats, score, false_positive)
- [x] HookBus fires on session end + observation save
- [x] HookBus circuit breaker disables after 3 consecutive failures
- [x] Pattern detections saved with modelId
- [x] Fingerprint computed at session end
- [x] Evolution runs every 5th session
- [x] Feedback adjusts confidence
- [x] False positive reduces pattern confidence by 15%
- [x] Patterns auto-deactivate at useCount < 2 (learned patterns)
- [x] DB pruning runs every 10th session
- [x] `ai/memory/insights.md` auto-generated (atomic write)
- [x] `ai/memory/state-dump.sql` auto-generated
- [x] `pnpm mcp:typecheck && pnpm mcp:build` — clean
