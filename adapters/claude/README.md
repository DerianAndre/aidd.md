...
Follow all rules defined in `rules/global.md`.
Consult `skills/` for specialized capabilities.
```
...
- **Rules** (`rules/`): Immutable constraints Claude Code must follow
- **Skills** (`skills/<name>/SKILL.md`): Specialized capabilities with model tier hints
- **Workflows** (`workflows/`): Multi-step procedures for complex tasks
...
2. AIDD skill scripts in `skills/<name>/scripts/` can be registered as MCP tools
